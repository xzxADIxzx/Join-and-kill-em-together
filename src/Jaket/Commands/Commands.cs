...
public static class Commands
{
    ...
    public static void Load()
    {
        ...
        static void Receive(string content) => UI.Chat.Receive(content);
        ...
        Handler.Register("hello", "Resend the tips for new players", args => UI.Chat.SayHello());

        Handler.Register("tts-volume", "\\[0-100]", "Set the volume of TTS", args =>
        {
            if (args.Length == 0)
                Receive($"[orange]TTS volume is {Settings.Volume}.");

            else if (int.TryParse(args[0], out int value))
                Receive($"[green]TTS volume is set to {Settings.Volume = Mathf.Clamp(value, 0, 100)}.");

            else
                Receive("[red]Couldn't parse the value, it must be an integer between 0 and 100.");
        });
        ...
        Handler.Register("tts", "\\[on/off]", "Toggle TTS for thy messages", args =>
        {
            if (args.Length > 0 ? args[0] == "on" : !Settings.AutoTTS)
            {
                ...
                Receive("[green]TTS enabled.");
            }
            else
            {
                Settings.AutoTTS = false;
                Receive("[red]TTS disabled.");
            }
        });
        ...
        Handler.Register("plushie", "<name>", "Spawn a plushie by name", args =>
        {
            if (args.Length == 0)
            {
                Receive("[red]No plushie name was provided.");
                return;
            }
        ...
        Handler.Register("level", "<layer> <level> / sandbox / cyber grind / museum", "Load a level", args =>
        {
            ...
            if (LobbyController.Online && !LobbyController.IsOwner) Receive("[red]Only the owner of the lobby can load levels.");

            else if (args.Length >= 1 && args[0].Length >= 2 && "sandbox sbtest".Contains(args[0]))
            {
                LoadScn("uk_construct");
                Receive("[green]Sandbox is loading...");
            }
            else if (args.Length >= 1 && args[0].Length >= 2 && "cyber grind cg".Contains(args[0]))
            {
                LoadScn("Endless");
                Receive("[green]The Cyber Grind is loading...");
            }
            else if (args.Length >= 1 && args[0].Length >= 2 && "credits museum".Contains(args[0]))
            {
                LoadScn("CreditsMuseum2");
                Receive("[green]Museum is loading...");
            }
            else if (args.Length < 2)
                Receive("[red]Insufficient number of arguments.");
            else if
            (
        ...
        ([
            ...
            "Translators",        "Poyozito, Fraku, Theoyeah, Nocturne, Doomguy, Fenicemaster, sSAR, \n[coral]|[] Becon, xzxADIxzx, NotPhobos, Repenkos, Sowler",
