...
public class PlayerInformation : Fragment
{
    ...
    public void Build() => Builder.Canvas(Create("Info", hc.transform).transform, default, c =>
    {
        ...
        Events.Post(() => { if (root) root.GetComponentsInChildren<Graphic>().Each(g => g.material = hc.fistFill.material); });
    });
    ...
    public void Build(RemotePlayer player, Transform root)
    {
        ...
        Text Text(TextAnchor align) => Builder.Text(Builder.Rect("Text", root, new() { Width = -32f }), "", 40, white, align);
        Text
        ...
        root.Add<Bar>(b => b.Update(() =>
        {
            ...
            playername.text = $"<color={(health > 0 ? White : Red)}>{player.Name}</color>";
