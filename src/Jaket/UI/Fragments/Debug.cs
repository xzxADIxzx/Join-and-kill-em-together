...
public class Debug : Fragment
{
    ...
    public Debug(Transform root) : base(root, "Debug", false) => Component<Bar>(Rect("Content", new(0f, 190f, 1920f, 380f, new(.5f, 0f))).gameObject, b =>
    {
        ...
        b.Subbar(136f, s =>
        {
            ...
            {
                b.Setup(true);
                b.Text("BYTES READ   ", 24f, out (readBs   = new(19)).Label, color: green);
                b.Text("BYTES SENT   ", 24f, out (sentBs   = new(19)).Label, color: Darker(green));
                b.Text("READ TIME    ", 24f, out (readMs   = new(96)).Label, color: orange);
                b.Text("WRITE TIME   ", 24f, out (writeMs  = new(96)).Label, color: Darker(orange));
            });
            Component<Bar>(s.Image(Tex.Fill, 320f, semi, multiplier: 2).gameObject, b =>
            {
                b.Setup(true);
                b.Text("ENTITY UPDATE", 24f, out (entityMs = new(96)).Label, color: blue);
                b.Text("TARGET UPDATE", 24f, out (targetMs = new(96)).Label, color: Darker(blue));
                b.Text("TOTAL TIME   ", 24f, out (totalMs  = new(96)).Label, color: purple);
                b.Text("FLUSH TIME   ", 24f, out (flushMs  = new(96)).Label, color: Darker(purple));
            });
        });
    });
    ...
    public class Data
    {
        ...
        public void Project(float peak)
        {
            ...
            for (int i = 0; i < data.Length; i++) o[i] = new(x += 16f, data[(start + i) % data.Length] / peak);
